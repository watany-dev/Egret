# review-codecommit → Lecs 移植プラン

## 概要

review-codecommit（TypeScript/Bun/Ink）のCLAUDE.mdとskills（update-design, update-docs）をLecs（Rust/Cargo）向けに適応・移植する。

---

## 1. CLAUDE.md の拡張

現在のLecsのCLAUDE.mdは基本情報（Overview, Build, Architecture, Dependencies）のみ。以下のセクションを追加する。
...
#### 1-2. プロジェクト基本方針
review-codecommitの方針をLecs向けに書き換え：
...
review-codecommitの設計書評価・改善スキルをLecs向けに適応：

**変更点**:
- frontmatterのdescriptionをLecsに合わせる
- Phase 1: `docs/design/*.md`の参照先はそのまま維持（Lecsでもdocs/配下に設計書を置く想定）
...
review-codecommitのドキュメント一括最新化スキルをLecs向けに適応：

**変更点**:
- Phase 2の対象ドキュメント:
  - 設計書: `docs/design/<feature>.md`（同じ構成）
  - 要件定義書: `docs/requirements.md`（まだ存在しないが、将来用に設定）
  - ロードマップ: `docs/ROADMAP.md`（Lecs固有、更新対象に追加）
- Phase 3: README.md更新
