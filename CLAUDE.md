...
Lecs is a local ECS task runner — run ECS task definitions locally by satisfying the runtime contract ECS apps expect (metadata endpoints, credential providers, dependsOn, health checks), without recreating the ECS control plane.
...
3. `test` — cargo test
4. `doc` — cargo doc with `-D warnings`
5. `deny` — cargo-deny (vulnerabilities, licenses, bans, sources)
...
- **安全性重視**: `unsafe` コード禁止、clippy pedantic/nursery/cargo 準拠、`unwrap` 使用禁止（deny）
- **カバレッジ**: 95% 以上を義務化（cargo-tarpaulin）
- **脆弱性チェック**: cargo-deny による依存関係の脆弱性・ライセンス監査
- **OCI ランタイム互換**: bollard クレートを通じた Docker/Podman API との連携
- **テスト品質**: コンテナランタイム API はモック（`ContainerRuntime` トレイト）、統合テストで E2E 確認
