...
| ID | 要件 | 状態 |
|----|------|------|
| FR-7.1 | `ECS_CONTAINER_METADATA_URI_V4` エンドポイントをモックする | 🔲 未実装 |
| FR-7.2 | タスクメタデータ JSON を返す | 🔲 未実装 |
| FR-7.3 | クレデンシャルプロバイダ（`/creds`, `/role/{name}`）をモックする | 🔲 未実装 |
| FR-7.4 | 各アプリコンテナに環境変数を自動注入する | 🔲 未実装 |
