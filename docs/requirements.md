...
| ID | 要件 | 状態 |
|----|------|------|
| FR-11.1 | `egret ps` でリソース使用量・ヘルスチェック状態・依存関係を表示できる | ✅ 実装済み |
| FR-11.2 | `egret ps` の出力形式を `--output json/wide` で切り替えできる | ✅ 実装済み |
| FR-11.3 | `egret inspect` で実行中タスクの詳細（実効設定、ネットワーク構成）を表示できる | ✅ 実装済み |
| FR-11.4 | `egret stats` でライブリソース使用量（CPU、メモリ、I/O）を表示できる | ✅ 実装済み |
| FR-11.5 | `egret history` で実行履歴を記録・表示できる | ✅ 実装済み |
...
| ID | 要件 | 状態 |
|----|------|------|
| FR-12.1 | `egret watch` でファイル変更時にタスクを自動再起動できる | ✅ 実装済み |
| FR-12.2 | `egret diff` でタスク定義をセマンティックに比較できる（カラー出力対応） | ✅ 実装済み |
| FR-12.3 | `--profile` で設定プロファイル（override + secrets）を切り替えできる | ✅ 実装済み |
| FR-12.4 | `egret completions` でシェル補完スクリプト（bash/zsh/fish）を生成できる | ✅ 実装済み |
