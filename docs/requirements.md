...
| FR-9.3 | `egret ps` で実行中タスク一覧を表示する | ✅ 実装済み |
| FR-9.4 | `egret logs <container>` で特定コンテナのログを表示する | ✅ 実装済み |

### FR-10: バリデーション + プロジェクト初期化

| ID | 要件 | 状態 |
|----|------|------|
| FR-10.1 | `egret validate` でタスク定義を静的解析できる（イメージ形式、ポート競合、ARN形式等） | ✅ 実装済み |
| FR-10.2 | `egret init` でスターターファイル（タスク定義、override、secrets テンプレート）を生成できる | ✅ 実装済み |
