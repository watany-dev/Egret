...
### FR-18: CloudFormation YAML + CDK 自動探索

| ID | 要件 | 状態 |
|----|------|------|
| FR-18.1 | `--from-cfn template.yaml` で YAML 形式の CloudFormation テンプレートを読み込める | ✅ 実装済み |
| FR-18.2 | ファイル拡張子（`.yaml`/`.yml`/`.json`）でフォーマットを自動検出する | ✅ 実装済み |
| FR-18.3 | 不明な拡張子の場合は JSON → YAML の順でフォールバック試行する | ✅ 実装済み |
| FR-18.4 | YAML カスタムタグ（`!Ref`, `!Sub` 等）を含むテンプレートを検出してエラー報告する | ✅ 実装済み |
| FR-18.5 | `--from-cdk <dir>` で CDK 出力ディレクトリから ECS タスク定義を自動探索できる | ✅ 実装済み |
| FR-18.6 | `--cdk-resource <logical_id>` で複数リソースから1つを明示的に選択できる | ✅ 実装済み |
| FR-18.7 | 単一テンプレートの場合は自動選択、複数の場合は ECS リソースを持つものを絞り込む | ✅ 実装済み |

### FR-19: ネットワークモード拡張

| ID | 要件 | 状態 |
|----|------|------|
| FR-19.1 | `networkMode: host` のタスク定義でコンテナがホストネットワークを共有して起動できる | ✅ 実装済み |
| FR-19.2 | `networkMode: none` のタスク定義でネットワークなしでコンテナが起動できる | ✅ 実装済み |
| FR-19.3 | `networkMode: awsvpc` は bridge として扱い、警告を出す | ✅ 実装済み |
| FR-19.4 | `networkMode` 未指定時はデフォルト bridge として動作する（既存動作維持） | ✅ 実装済み |
| FR-19.5 | host モードではメタデータ URI に `127.0.0.1` を使用する | ✅ 実装済み |
| FR-19.6 | none モードではメタデータ/クレデンシャル URI を注入しない | ✅ 実装済み |
| FR-19.7 | host モードでの `hostPort` ≠ `containerPort` に対してバリデーション警告を出す | ✅ 実装済み |
