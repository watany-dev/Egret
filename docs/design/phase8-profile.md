...
Phase 8 の設定プロファイル機能は、環境ごと（dev/staging/prod）に異なる override ファイルや secrets ファイルを規約ベースで自動ロードする。`--profile dev` で `lecs-override.dev.json` / `secrets.dev.json` を自動解決し、`.lecs.toml` でデフォルトプロファイルを設定可能にする。
...
- `-f` フラグ（タスク定義パス）は常に必須であり、`.lecs.toml` での上書きはスコープ外
- 未知フィールドは無視する（前方互換性）
...
|--------------|-----------------|----------------|
| `dev` | `lecs-override.dev.json` | `secrets.dev.json` |
| `staging` | `lecs-override.staging.json` | `secrets.staging.json` |
| `prod` | `lecs-override.prod.json` | `secrets.prod.json` |
...
pub fn effective_profile<'a>(cli_profile: Option<&'a str>, config: Option<&'a LecsConfig>) -> Option<&'a str>;
...
2. profile::load_config_with_warning(base_dir)
     → .lecs.toml を上方探索、パース失敗時は tracing::warn! + None
3. profile::effective_profile(args.profile, config)
     → args.profile (CLI) > config.default_profile (.lecs.toml) > None
...
| 3 | `.lecs.toml` の `default_profile` | 暗黙のデフォルト |
...
| ケース | 挙動 | 理由 |
|--------|------|------|
| `.lecs.toml` 読み込み/パース失敗 | `tracing::warn!` + 無視（`None` にフォールバック） | 設定ファイルの欠落はデフォルト動作 |
...
```
lecs run -f <file> [--profile <name>] [--override <file>] [--secrets <file>]
lecs validate -f <file> [--profile <name>] [--override <file>] [--secrets <file>]
```
...
| テスト種別 | テスト数 | 対象 |
|-----------|---------|------|
| `LecsConfig` パース | 5 | `from_toml`, `from_file`, エラーケース |
