...
- `ContainerRuntime` トレイト + `MockContainerClient` 導入済み（テスタブルな設計）
...
- `ContainerDefinition` を直接構築するテスト箇所:
  ...
  - `src/cli/stop.rs`: `ContainerDefinition` 直接構築なし（ランタイムモックのみ）
...
4. テスト用 fixture 追加:
   ...
   - `tests/fixtures/lecs-override.json`
...
### 手動テスト（Docker/Podman 環境がある場合）
```bash
# Override テスト
lecs run -f tests/fixtures/simple-task.json --override tests/fixtures/lecs-override.json

# Secrets テスト
lecs run -f tests/fixtures/task-with-secrets.json --secrets tests/fixtures/secrets.local.json

# 両方組み合わせ
lecs run -f tests/fixtures/task-with-secrets.json \
  --override tests/fixtures/lecs-override.json \
