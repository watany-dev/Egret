...
- **`lecs validate`** — タスク定義の静的解析（collect-all 方式の構造化診断）
- **`lecs init`** — スターターファイル生成（テンプレートスキャフォールディング）
...
```
lecs validate -f task-def.json [--override ...] [--secrets ...]
    │
    ▼
...
lecs init [--dir .] [--image ...] [--family ...]
    │
    ▼
generate_task_definition() → task-definition.json
generate_override_template() → lecs-override.json
generate_secrets_template() → secrets.local.json

lecs run -f task-def.json --dry-run
    │
    ▼
...
| `src/cli/validate.rs` | `lecs validate` コマンド実装 |
| `src/cli/init.rs` | `lecs init` コマンド実装 |
...
1. 出力ディレクトリの存在確認
2. 各ファイル（`task-definition.json`, `lecs-override.json`, `secrets.local.json`）の存在チェック
3. 存在しないファイルのみ生成、既存ファイルはスキップ + メッセージ表示
4. 次ステップのヒント表示（`lecs validate`, `lecs run`）
