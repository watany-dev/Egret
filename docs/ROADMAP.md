...
├── cli/                 # CLI コマンド定義 (run, stop, ps, logs, init, validate, inspect, stats, history, completions, diff, version)
├── taskdef/             # Task Definition JSON パーサ・型定義・診断
...
Phase 0-2.5: ✅ 完了
    ...
    │       ├── Phase 7 (可観測性) ✅
    │       │
    │       └── Phase 8 (ワークフロー高速化) ← Phase 7 と並行可
```
